...
def align_depth_to_rgb(
    depth_image, pixel_coords, inverse_depth_intrinsic, rgb_intrinsic, depth_to_rgb_extrinsic, rgb_width, rgb_height
):
    """
...
def quantize_depth_frame(frame, depth_scale_factor):
    """
    Further quantize the depth image for nice visualization depth_scale_factor == value used to normalize depth image

    Returns:
        Quantized depth map image
    """
    return cv2.applyColorMap(cv2.convertScaleAbs(frame.astype(float), alpha=255 / depth_scale_factor), cv2.COLORMAP_JET)


def overlay_depth_frame(rgb_frame, depth_frame, rgb_alpha):
    """
